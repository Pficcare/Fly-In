...
class PrinceOfParser:
    ...
    def split_not_spit(self):
        ...
        self.hub:dict = {"coordo": (0,0), "zone": "normal", "color": None, "max_drones": 1}
        ...
        for line in self.instruction.strip().splitlines():
            ...
            if hub == "start_hub" or hub == "end_hub":
                id_node = self.start_hub.copy() # sans cpy,on passe l adresse a id donc le meme dict
                id_node["voisin"] = {}
                if hub == "end_hub":
    ...
    def convertion(self, x: str, y:str) -> tuple[int,int]:
        try:
            a = int(x)
            b = int(y)
